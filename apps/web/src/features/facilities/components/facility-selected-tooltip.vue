<script setup lang="ts">
  ...
  const fullAddress = computed(() => {
    const parts: string[] = [];
    if (props.state.address) parts.push(props.state.address);
    if (props.state.city) parts.push(props.state.city);
    if (props.state.stateAbbrev) parts.push(props.state.stateAbbrev);
    if (parts.length === 0 && props.state.facilityName) return props.state.facilityName;
  ...
  const toggleBgClass = computed(() => {
    if (!nearbyInfrastructure.value) return "bg-[#cbd5e1]";
    ...
    <div class="flex min-w-[376px] flex-col gap-4 rounded-[8px] bg-white p-4 shadow-md">
      ...
      <div class="flex flex-col gap-2">
        ...
        <div class="flex items-start justify-between gap-4">
          <div class="flex flex-wrap items-center gap-2 leading-none">
            ...
            <span v-if="codeText" class="text-[16px] font-normal whitespace-nowrap text-[#94a3b8]">
              {{ codeText }}
            </span>
          </div>
          <button
        ...
        <div class="flex flex-col">
          <span v-if="marketLine" class="text-[16px] font-normal leading-normal" :class="accentText">
            {{ marketLine }}
          </span>
      ...
      <div class="flex flex-col gap-6">
        ...
        >
          ...
          >
            <span class="pl-2 text-[12px] font-normal uppercase leading-normal" :class="accentText">
              {{ section.label }}
            </span>
