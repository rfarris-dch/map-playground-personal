...
function LoadingIcon(props: React.ComponentPropsWithoutRef<"svg">) {
  ...
  return (
    <svg aria-hidden="true" fill="none" viewBox="0 0 20 20" {...props}>
      ...
      />
      <defs>
        <linearGradient gradientUnits="userSpaceOnUse" id={id} x1="13" x2="9.5" y1="9" y2="15">
...
function SearchResult({
  ...
  return (
    ...
    >
      <div
        aria-hidden="true"
        className="text-slate-700 text-sm group-aria-selected:text-sky-600 dark:text-slate-300 dark:group-aria-selected:text-sky-400"
        id={`${id}-title`}
      >
      ...
      {hierarchy.length > 0 && (
        <div
          aria-hidden="true"
          className="mt-0.5 truncate whitespace-nowrap text-slate-500 text-xs dark:text-slate-400"
          id={`${id}-hierarchy`}
        >
...
function SearchResults({
  ...
  if (collection.items.length === 0) {
    return (
      <p className="px-4 py-8 text-center text-slate-700 text-sm dark:text-slate-400">
        No results for &ldquo;
        <span className="wrap-break-word text-slate-900 dark:text-white">{query}</span>
        &rdquo;
      </p>
    );
  }
          ...
          <DialogPanel className="mx-auto transform-gpu overflow-hidden rounded-xl bg-white shadow-xl sm:max-w-xl dark:bg-slate-800 dark:ring-1 dark:ring-slate-700">
            <div {...autocomplete.getRootProps({})}>
              ...
              >
                ...
                />
                <div
                  className="border-slate-200 border-t bg-white px-2 py-3 empty:hidden dark:border-slate-400/10 dark:bg-slate-800"
                  ref={panelRef}
