...
- **Chat** (`chat_send`): Deep analytical reasoning over **the current file selection**. It sees selected files **completely** (full content, not summaries), but it **only sees what's in the selection** — nothing else. It excels at synthesizing patterns, spotting architectural issues, and forming hypotheses from the big picture.
...
5. **Document findings as you go** — create/update a report file with observations
...
Update the selection to focus the chat on what matters now, then ask targeted questions:
...
**Repeat Phases 3–4** as needed, but be judicious. The chat is slow and resource-intensive — do substantial reasoning and evidence gathering on your own between calls. Don't invoke it just to ask a quick question you could answer yourself by reading a file. Reserve it for moments when you need deep analytical synthesis across the selected context.
...
- 🚫 Invoking the chat for questions you could answer by reading a file yourself — reserve it for deep analytical synthesis
