...
export function ThemeSelector(props: React.ComponentPropsWithoutRef<typeof Listbox<"div">>) {
  ...
  return (
    <Listbox as="div" onChange={setTheme} value={theme} {...props}>
      ...
      <ListboxButton
        aria-label="Theme"
        className="flex h-6 w-6 items-center justify-center rounded-lg shadow-black/5 shadow-md ring-1 ring-black/5 dark:bg-slate-700 dark:ring-white/5 dark:ring-inset"
      >
        <LightIcon
          className={clsx(
            "h-4 w-4 dark:hidden",
      ...
      <ListboxOptions className="absolute top-full left-1/2 mt-3 w-36 -translate-x-1/2 space-y-1 rounded-xl bg-white p-3 font-medium text-sm shadow-black/5 shadow-md ring-1 ring-black/5 dark:bg-slate-800 dark:ring-white/5">
        {themes.map((theme) => (
          <ListboxOption
