...
<template>
  <div v-if="props.active" class="pointer-events-none absolute inset-0 z-20">
    <div
      v-if="hiddenCount > 0"
      class="map-glass-subtle absolute right-4 top-4 rounded-full px-2 py-1 text-[10px] font-medium text-muted-foreground"
    >
    ...
    <article
      v-for="card in cards"
      :key="card.id"
      class="map-glass-surface absolute max-w-[190px] rounded-md px-2 py-1.5 text-[11px]"
      :style="{
