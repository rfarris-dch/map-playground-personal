...
<template>
  <article class="space-y-4 text-[#94A3B8]">
    ...
    <div v-else-if="hasCountySummary" class="space-y-4">
      ...
      <div class="grid gap-x-4 gap-y-2 text-[10px] sm:grid-cols-2 xl:grid-cols-5">
        <div>
          <div class="uppercase tracking-wide text-[#94A3B8]">Selected Counties</div>
          <div class="text-lg font-semibold text-[#64748B]">{{ requestedCountyIds.length }}</div>
        </div>
        <div>
          <div class="uppercase tracking-wide text-[#94A3B8]">Ranked</div>
          <div class="text-lg font-semibold text-[#64748B]">{{ rankedCountyCount }}</div>
        </div>
        <div>
          <div class="uppercase tracking-wide text-[#94A3B8]">Deferred</div>
          <div class="text-lg font-semibold text-[#64748B]">{{ deferredCountyIds.length }}</div>
        </div>
        <div>
          <div class="uppercase tracking-wide text-[#94A3B8]">Blocked</div>
          <div class="text-lg font-semibold text-[#64748B]">{{ blockedCountyIds.length }}</div>
        </div>
        <div>
          <div class="uppercase tracking-wide text-[#94A3B8]">Missing Geography</div>
      ...
      <div v-if="hasRows" class="space-y-6">
        <div
          v-for="row in rows"
          :key="row.countyFips"
          class="space-y-3 border-t border-[#E2E8F0]/60 pt-4"
        >
          <div class="flex flex-wrap items-start justify-between gap-3">
            ...
            <div class="flex flex-wrap items-center gap-2 text-[10px]">
              <span
                class="rounded-[4px] border px-2.5 py-1 font-medium"
          ...
          <!-- Key scores — flat row -->
          <div class="grid gap-x-4 gap-y-2 text-[10px] sm:grid-cols-2 xl:grid-cols-4">
            <div class="rounded-[4px] bg-[#F8FAFC] px-2 py-1.5">
              <div class="uppercase tracking-wide text-[#94A3B8]">Pressure Index</div>
              <div class="text-sm font-semibold text-[#64748B]">
                {{ formatMetric(row.marketPressureIndex) }}
              </div>
            </div>
            <div class="rounded-[4px] bg-[#F8FAFC] px-2 py-1.5">
              <div class="uppercase tracking-wide text-[#94A3B8]">Tier</div>
              <div class="text-sm font-semibold text-[#64748B]">
                {{ formatTier(row.attractivenessTier) }}
              </div>
            </div>
            <div class="rounded-[4px] bg-[#F8FAFC] px-2 py-1.5">
              <div class="uppercase tracking-wide text-[#94A3B8]">Freshness</div>
              <div class="text-sm font-semibold text-[#64748B]">
                {{ formatMetric(row.freshnessScore) }}
              </div>
            </div>
            <div class="rounded-[4px] bg-[#F8FAFC] px-2 py-1.5">
              <div class="uppercase tracking-wide text-[#94A3B8]">Updated</div>
              <div class="text-sm font-semibold text-[#64748B]">
          ...
          <!-- Narrative -->
          <div class="text-[10px]">
            <div class="uppercase tracking-wide text-[#94A3B8]">Narrative</div>
          ...
          <dl class="grid gap-x-4 gap-y-2 text-[10px] sm:grid-cols-2 xl:grid-cols-4">
            <div class="rounded-[4px] bg-[#F8FAFC] px-2 py-1.5">
          ...
          <dl class="grid gap-6 text-[10px] lg:grid-cols-3">
            <div>
              <dt class="uppercase tracking-wide text-[#94A3B8]">Demand</dt>
            ...
            <div>
              <dt class="uppercase tracking-wide text-[#94A3B8]">Supply + Friction</dt>
              <dd class="mt-1 space-y-1 text-[#64748B]">
            ...
            <div>
              <dt class="uppercase tracking-wide text-[#94A3B8]">Policy + Context</dt>
              <dd class="mt-1 space-y-1 text-[#64748B]">
          ...
          <div
            ...
            class="grid gap-6 text-[10px] lg:grid-cols-3"
          >
            <div>
              <div class="uppercase tracking-wide text-[#94A3B8]">Top Drivers</div>
            ...
            <div>
              <div class="uppercase tracking-wide text-[#94A3B8]">Deferred Reasons</div>
            ...
            <div>
              <div class="uppercase tracking-wide text-[#94A3B8]">What Changed</div>
