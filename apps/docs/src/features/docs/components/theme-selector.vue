<script setup lang="ts">
  ...
  interface ThemeOption {
    readonly icon: "light" | "dark" | "system";
  ...
  const themeOptions = shallowRef<readonly ThemeOption[]>([
    { name: "Light", value: "light", icon: "light" },
    { name: "Dark", value: "dark", icon: "dark" },
    { name: "System", value: "system", icon: "system" },
  ]);

  const buttonIconFillClass = computed(() => {
    return currentTheme.value === "system" ? "fill-slate-400" : "fill-sky-400";
  });
  ...
  function resolveOptionIconClass(selected: boolean): string {
    return selected ? "fill-sky-400 dark:fill-sky-400" : "fill-slate-400";
  }
</script>
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <svg
              v-if="themeOption.icon === 'light'"
              aria-hidden="true"
              viewBox="0 0 16 16"
              class="h-4 w-4"
              :class="resolveOptionIconClass(selected)"
            >
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
            ...
            <svg
              v-else-if="themeOption.icon === 'dark'"
              aria-hidden="true"
              viewBox="0 0 16 16"
              class="h-4 w-4"
              :class="resolveOptionIconClass(selected)"
