...
import type {
  SketchMeasureFeature,
  SketchMeasureSourceData,
} from "@/features/sketch-measure/sketch-measure.service.types";
import type {
  SketchAreaGeometry,
...
function areaCursorVertex(runtime: SketchMeasureRuntimeState): LngLat | null {
...
function areaEdgeVertex(runtime: SketchMeasureRuntimeState): LngLat | null {
...
function buildAreaRing(runtime: SketchMeasureRuntimeState): LngLat[] {
...
function buildAreaLineVertices(
  runtime: SketchMeasureRuntimeState,
...
function canFinishArea(runtime: SketchMeasureRuntimeState): boolean {
...
function buildAreaGeometry(
  areaShape: SketchAreaGeometry["areaShape"],
  ring: readonly LngLat[],
  areaSqKm: number | null,
  distanceKm: number | null
): SketchAreaGeometry | null {
  if (ring.length < 4) {
    return null;
  }
  ...
  return {
    ...
    areaSqKm,
    distanceKm,
    ring: cloneVertices(ring),
  };
}

export function emptySketchMeasureSourceData(): SketchMeasureSourceData {
  return {
...
export function buildSketchMeasureState(runtime: SketchMeasureRuntimeState): SketchMeasureState {
  ...
  const areaSqKm = runtime.mode === "area" ? polygonAreaSqKm(areaRing) : null;
  const draftAreaGeometry =
    runtime.mode === "area"
      ? buildAreaGeometry(runtime.areaShape, areaRing, areaSqKm, distanceKm)
      : null;

  return {
    areaShape: runtime.areaShape,
    areaSqKm,
    canFinishArea: canFinishArea(runtime),
    completedAreaGeometry: runtime.areaComplete ? draftAreaGeometry : null,
    distanceKm,
    draftAreaGeometry,
    isAreaComplete: runtime.mode === "area" && runtime.areaComplete,
    mode: runtime.mode,
    vertexCount: runtime.vertices.length,
  };
}

export function buildSketchMeasureSourceData(
  runtime: SketchMeasureRuntimeState
): SketchMeasureSourceData {
  if (runtime.mode === "off") {
    return emptySketchMeasureSourceData();
  }

  const features: SketchMeasureFeature[] = [];
  const areaRing = buildAreaRing(runtime);
